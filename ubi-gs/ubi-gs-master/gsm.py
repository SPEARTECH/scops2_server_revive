...
class GSMessageHeader:
  ...
  def __init__(self, bts: bytes):
    ...
    try:
      ...
      # Unknown message type — store raw value so callers can inspect it
      self.type = bts[4]
  ...
  def __bytes__(self):
    ...
    result[4] = self.type.value if isinstance(self.type, MESSAGE_TYPE) else self.type
    result[5] &= 0xF
...
class Message:
  ...
  def __repr__(self):
    payload = self.dl or ""
    type_name = self.header.type.name if isinstance(self.header.type, MESSAGE_TYPE) else f"0x{self.header.type:02X}"
    return f"<{type_name}\t{self.header.property.name}\t{self.header.sender.name}->{self.header.receiver.name}\t{self.header.size}B>\n{payload}"
...
class GSMessageBundle:
  ...
  def __repr__(self):
    def _tname(t):
      return t.name if isinstance(t, MESSAGE_TYPE) else f"0x{t:02X}"
    result = f"<BUNDLE: {_tname(self.msgs[0].header.type)}"
    for i in range(1, len(self.msgs), 1):
      result += f" + {_tname(self.msgs[i].header.type)}"
    return result + ">"
...
class GSMResponse:
  ...
  def __repr__(self):
    payload = self.dl or ""
    type_name = self.header.type.name if isinstance(self.header.type, MESSAGE_TYPE) else f"0x{self.header.type:02X}"
    return f"<{type_name} RES\t{self.header.property.name}\t{self.header.sender.name}->{self.header.receiver.name}\t{self.header.size}B>\n{payload}"
